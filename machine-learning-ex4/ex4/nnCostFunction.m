...
% Create hypothesis - forward propagation - ZEROS IN THE BEGINNING!
...
% Re-code output according to the output layer
decade = eye(num_labels);
y_vec = zeros(m, num_labels);
for i = 1:m
  y_vec(i,:) = decade(y(i),:);
endfor

% Calculate cost. Easy peasy.
J = sum(sum((-y_vec) .* log(h) - (1 - y_vec) .* log(1 - h))) / m;

% Regularization added to degrade those unworthy attributes
